...
namespace FindstakeNet
{
    public class RPCClient
    {
        ...
        {
            return await RpcCall<RawTransactionResponse>(new RPCRequest("getrawtransaction", [txId, verbose]));
        }

        public async Task<TransactionResponse> GetTransaction(string txId)
        {
            return await RpcCall<TransactionResponse>(new RPCRequest("gettransaction", [txId]));
        }

        public async Task<DecodeRawTransactionResponse> DecodeRawTransaction(string transaction)
        {
            return await RpcCall<DecodeRawTransactionResponse>(new RPCRequest("decoderawtransaction", [transaction]));
        }

        public async Task<BlockResponse> GetBlock(string hash)
        {
            return await RpcCall<BlockResponse>(new RPCRequest("getblock", [hash]));
        }
        ...
        {
            return await RpcCall<string>(new RPCRequest("getblockhash", [index]));
        }
        ...
        public async Task<string> CreateRawCoinStakeTransaction(IReadOnlyList<RawTxStakeInputs> inputs, IReadOnlyList<RawTxStakeOutput> outputs, long timestamp)
        {
            ...
            return await RpcCall<string>(new RPCRequest("createrawtransaction", [param1, param2, 0, timestamp]));
        }

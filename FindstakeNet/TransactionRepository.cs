namespace FindstakeNet
{
	...
	{
		private readonly object listAdrlock = new();
		private List<AddressTxoState> addresses;

		private readonly object listTxlock = new();
		private List<TxState> transactions;

		private readonly object listOutputslock = new();
		...
		public List<UnspentTransactions> GetUnspents(string address)
		{
			...
			if (txStates.Count > 0)
            {
                var blockStates = blockRepository.GetBlockStates(txStates.Select(txState => txState.height).ToList());

...
namespace FindstakeNet
{
	public static class Mint
	{
		public static ulong? GetModifier(IReadOnlyList<StakeModifier> blockModifiers, uint curtime, uint findstakelimit)
		{
			...
			foreach (var blockmodifier in blockModifiers)
            {
                if (blockmodifier.bt <= delta)
                {
                    match = blockmodifier;
                }
                else
		...
		public static float CompactToDiff(uint bits)
		{
		...
		public static uint GetExponent(BigInteger n)
		{
			...
			if (arr.Length > 1 && arr[exponent - 1] == 0 && arr[exponent - 2] == 128)
			{
				exponent--;
			}
